...
class DQNAgent:
    ...
    def build_agent_model(self):  # 행동을 결정하는 인공신경망
        model = Sequential()
        model.add(Dense(24, input_dim=self.state_size + 1, activation='relu', kernel_initializer='he_uniform')) # 여기서는 상대의 예상행동도 인수로 받기때문에 state+1
    ...
    def train_model(self):  # 학습시키기
        ...
        for i in range(self.batch_size):
            if done[i]:
                ...
                target[i][actions[i]] = rewards[i] + self.discount_factor * (np.amax(target_val[i])) #Todo 모델 학습에 적 행동 유추하는 모델도 학습시키기 까먹으면 안되지
